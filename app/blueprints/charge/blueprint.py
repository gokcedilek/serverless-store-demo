...
import os
from middlewares.auth import auth_required, verify_firebase_id_token
    ...
    with tracer.start_as_current_span("charge_endpoint", kind=trace.SpanKind.PRODUCER) as root_span:
        ...
        with tracer.start_as_current_span("send_payment_event", kind=trace.SpanKind.PRODUCER) as child_span:
            ...
            if stripe_token:
                ...
                print("POST /charge endpoint sent order_id: ", order_id, " stripe_token: ", stripe_token)
                child_span.add_event(name="published order created event")

                # test UID decoding
                firebase_id_token = request.cookies.get('firebase_id_token')
                print("POST /charge endpoint firebase token: ", firebase_id_token)
                test_auth_context = verify_firebase_id_token(firebase_id_token)
                print("POST /charge endpoint auth context: ", test_auth_context)

                # test generating custom firebase id token
                print("!!!!!!!!POST /charge endpoint uid: ", uid)
                custom_token = auth.create_custom_token(uid)
                print("!!!!!!!POST /charge endpoint custom token: ", custom_token)
                test_auth_context_custom = verify_firebase_id_token(custom_token)
                print("POST /charge endpoint auth context custom: ", test_auth_context_custom)
